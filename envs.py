...
def make_env(env_id, seed, rank, log_dir, add_timestep, allow_early_resets):
    def _thunk():
        ...
        if log_dir is not None:
            env = bench.Monitor(env, filename=None, allow_early_resets=allow_early_resets)
...
def make_vec_envs(env_name, seed, num_processes, gamma, log_dir, add_timestep,
                  ...
                  norm_obs=True, norm_ret=True, render=False):
    envs = [make_env(env_name, seed, i, log_dir, add_timestep, allow_early_resets)
            for i in range(num_processes)]

    if len(envs) > 1:
        envs = SubprocVecEnv(envs, render=render)
    else:
